...
public class UrsaStorageConfig {
    ...
    public static UrsaStorageConfig fromConfigs(Map<String, ?> configs) throws Exception {
        boolean enabled = getStringConfig(configs, UrsaConfigDefaults.URSA_STORAGE_ENABLE_CONFIG,
                String.valueOf(UrsaConfigDefaults.URSA_STORAGE_ENABLE_DEFAULT)).equals("true");

        final var catalogOxiaServiceUrl = getOxiaServiceUrlConfig(
                configs, UrsaConfigDefaults.URSA_CATALOG_OXIA_SERVICE_URL_CONFIG,
                UrsaConfigDefaults.URSA_CATALOG_OXIA_SERVICE_URL_DEFAULT);
        final var ursaOxiaServiceUrl = getOxiaServiceUrlConfig(configs, UrsaConfigDefaults.URSA_OXIA_SERVICE_URL_CONFIG,
                UrsaConfigDefaults.URSA_OXIA_SERVICE_URL_DEFAULT);

        String backendType = getStringConfig(configs, UrsaConfigDefaults.URSA_STORAGE_BACKEND_TYPE_CONFIG,
                UrsaConfigDefaults.URSA_STORAGE_BACKEND_TYPE_DEFAULT);

        String storagePath = getStringConfig(configs, UrsaConfigDefaults.URSA_STORAGE_PATH_CONFIG,
                UrsaConfigDefaults.URSA_STORAGE_PATH_DEFAULT);

        String compactionPrefix = getStringConfig(configs, UrsaConfigDefaults.URSA_STORAGE_COMPACTION_PREFIX_CONFIG,
                UrsaConfigDefaults.URSA_STORAGE_COMPACTION_PREFIX_DEFAULT);

        long writeBufferFlushIntervalMs = getLongConfig(configs,
                UrsaConfigDefaults.URSA_STORAGE_WRITE_BUFFER_FLUSH_INTERVAL_MS_CONFIG,
                UrsaConfigDefaults.URSA_STORAGE_WRITE_BUFFER_FLUSH_INTERVAL_MS_DEFAULT);

        int writeBufferSize = getIntConfig(configs,
                UrsaConfigDefaults.URSA_STORAGE_WRITE_BUFFER_SIZE_CONFIG,
                UrsaConfigDefaults.URSA_STORAGE_WRITE_BUFFER_SIZE_DEFAULT);

        long writeBufferFlushSize = getLongConfig(configs,
                UrsaConfigDefaults.URSA_STORAGE_WRITE_BUFFER_FLUSH_SIZE_CONFIG,
                UrsaConfigDefaults.URSA_STORAGE_WRITE_BUFFER_FLUSH_SIZE_DEFAULT);

        String s3Endpoint = getStringConfig(configs, UrsaConfigDefaults.URSA_STORAGE_S3_ENDPOINT_CONFIG,
                UrsaConfigDefaults.URSA_STORAGE_S3_ENDPOINT_DEFAULT);

        String s3AccessKey = getStringConfig(configs, UrsaConfigDefaults.URSA_STORAGE_S3_ACCESS_KEY_CONFIG,
                UrsaConfigDefaults.URSA_STORAGE_S3_ACCESS_KEY_DEFAULT);

        String s3SecretKey = getStringConfig(configs, UrsaConfigDefaults.URSA_STORAGE_S3_SECRET_KEY_CONFIG,
                UrsaConfigDefaults.URSA_STORAGE_S3_SECRET_KEY_DEFAULT);

        String s3SessionToken = getOptionalStringConfig(configs,
                UrsaConfigDefaults.URSA_STORAGE_S3_SESSION_TOKEN_CONFIG);

        Boolean s3PathStyleAccess = getOptionalBooleanConfig(configs,
                UrsaConfigDefaults.URSA_STORAGE_S3_PATH_STYLE_ACCESS_CONFIG);

        String s3Bucket = getStringConfig(configs, UrsaConfigDefaults.URSA_STORAGE_S3_BUCKET_CONFIG,
                UrsaConfigDefaults.URSA_STORAGE_S3_BUCKET_DEFAULT);

        String compactionBucket = getStringConfig(configs, UrsaConfigDefaults.URSA_STORAGE_COMPACTION_BUCKET_CONFIG,
                s3Bucket);

        String s3Region = getStringConfig(configs, UrsaConfigDefaults.URSA_STORAGE_S3_REGION_CONFIG,
                UrsaConfigDefaults.URSA_STORAGE_S3_REGION_DEFAULT);

        long producerStateSnapshotIntervalMs = getLongConfig(configs,
                UrsaConfigDefaults.URSA_STORAGE_PRODUCER_STATE_SNAPSHOT_INTERVAL_MS_CONFIG,
                UrsaConfigDefaults.URSA_STORAGE_PRODUCER_STATE_SNAPSHOT_INTERVAL_MS_DEFAULT);

        int producerStateSnapshotRecordThreshold = getIntConfig(configs,
                UrsaConfigDefaults.URSA_STORAGE_PRODUCER_STATE_SNAPSHOT_RECORD_THRESHOLD_CONFIG,
                UrsaConfigDefaults.URSA_STORAGE_PRODUCER_STATE_SNAPSHOT_RECORD_THRESHOLD_DEFAULT);
        String classPath = getOptionalStringConfig(configs, UrsaConfigDefaults.URSA_STORAGE_CLASS_PATH_CONFIG);
