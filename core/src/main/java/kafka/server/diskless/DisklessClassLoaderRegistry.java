...
final class DisklessClassLoaderRegistry {
    ...
    private static void release(SharedRuntime runtime) {
        ...
        // gRPC closes shared executors asynchronously after client.close() returns. Their final
        // tasks may still load classes. Dropping the registry reference lets the JVM reclaim the
        // loader once those threads finish; eagerly closing its jar files breaks that cleanup.
    }
